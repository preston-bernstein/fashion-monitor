...
- **rebrowser-patches / Patchright / Camoufox** — candidate replacements that patch the CDP leak (Camoufox is an anti-detect Firefox). Status per `docs/playwright-stealth-pilot.md`: **deferred pilot, not wired** (proposed `PLAYWRIGHT_STEALTH_DRIVER=rebrowser|legacy` env does not exist yet). Hard fence in that doc: **do not remove playwright-extra + stealth until a rebrowser pilot passes live smoke on Depop/Poshmark.**
