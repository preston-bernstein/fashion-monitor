...
describe("AuditLogRepo", () => {
  ...
  it("records and fetches recent entries newest-first", () => {
    ...
    repo.record({ userId: 1, actorEmail: "a@example.com", action: "login.success" }, "2026-01-01T00:00:00.000Z");
    repo.record({ userId: 1, actorEmail: "a@example.com", action: "login.success" }, "2026-01-02T00:00:00.000Z");
  ...
  it("scopes fetchRecent to the repo's own profile", () => {
    ...
    repoA.record({ userId: 1, actorEmail: null, action: "login.success" }, "2026-01-01T00:00:00.000Z");
    repoB.record({ userId: 2, actorEmail: null, action: "login.success" }, "2026-01-01T00:00:00.000Z");
  ...
  it("fetchFiltered filters by category via its SQL mapping", () => {
    const repo = new AuditLogRepo(db, "default");
    repo.record({ userId: 1, actorEmail: null, action: "login.success" }, "2026-01-01T00:00:00.000Z");
    repo.record({ userId: 1, actorEmail: null, action: "secret.upsert" }, "2026-01-01T00:00:01.000Z");
    repo.record({ userId: 1, actorEmail: null, action: "search_group.create" }, "2026-01-01T00:00:02.000Z");
  ...
  it("fetchFiltered falls back to actionPrefix only when no category is given", () => {
    const repo = new AuditLogRepo(db, "default");
    repo.record({ userId: 1, actorEmail: null, action: "invite.create" }, "2026-01-01T00:00:00.000Z");
    repo.record({ userId: 1, actorEmail: null, action: "invite.redeem" }, "2026-01-01T00:00:01.000Z");
    repo.record({ userId: 1, actorEmail: null, action: "profile.delete" }, "2026-01-01T00:00:02.000Z");
  ...
  it("fetchFiltered filters by actorEmail and since, and paginates with total independent of limit", () => {
    const repo = new AuditLogRepo(db, "default");
    repo.record({ userId: 1, actorEmail: "a@example.com", action: "login.success" }, "2026-01-01T00:00:00.000Z");
    repo.record({ userId: 2, actorEmail: "b@example.com", action: "login.success" }, "2026-01-02T00:00:00.000Z");
    repo.record({ userId: 1, actorEmail: "a@example.com", action: "login.success" }, "2026-01-03T00:00:00.000Z");
  ...
  it("clamps limit to between 1 and 100", () => {
    const repo = new AuditLogRepo(db, "default");
    for (let i = 0; i < 5; i++) {
      repo.record({ userId: 1, actorEmail: null, action: "login.success" }, `2026-01-0${i + 1}T00:00:00.000Z`);
    }
