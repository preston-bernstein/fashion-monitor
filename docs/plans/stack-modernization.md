...
Most of the work in this plan is pulling the backend up to where the frontend already is, then unifying both halves on one toolchain.
...
fashion-monitor's Node.js version is old enough to need an upgrade before its security-fix window closes.

**Today:** `engines.node` requires `">=20"`, the `Dockerfile` uses `FROM node:20-bookworm`, and `@types/node` is pinned to `^22`.
...
- Move to **Node 24 LTS now**: set `engines.node` to `">=24"`, the Docker base image to `node:24-bookworm`, and `@types/node` to `^24`. Node 20 is already on the maintenance clock, and several of the toolchain upgrades below assume Node 22 or newer.
- Plan to ride **Node 26 into its October-2026 LTS promotion in place**. This needs no separate migration step: adopt 26 as the Current version near its LTS date, and stay on it.
- Before bumping the Docker base image, verify that the native modules (`better-sqlite3`, `argon2`) ship prebuilt binaries for the chosen Node ABI (Application Binary Interface — the low-level contract compiled native code depends on; when it changes, native modules must be rebuilt). They do, as of the versions listed below. A rebuild step using `python3`, `make`, and `g++` is already in the Dockerfile, and covers any gap.
...
fashion-monitor should keep its current SQLite driver rather than switch to Node's built-in one, which isn't stable yet.

**Today:** `better-sqlite3 ^11.8.1` is used synchronously throughout `src/storage/`, with hand-rolled, idempotent SQL migrations loaded at runtime from `dist/storage/migrations/*.sql`.
...
- **Bump `better-sqlite3` 11 → 12** and keep it. The API is stable across this major version for typical `prepare`/`run`/`all`/`exec` usage; review the changelog for the bundled SQLite version bump, since that occasionally changes default pragmas or `defensive` mode.
- **Do not migrate to `node:sqlite`.** It is not yet stable, and it would not buy anything here: this app already has a fast, synchronous driver and working migrations, and the synchronous, transaction-heavy code in `src/storage/repos/*` is exactly the pattern `better-sqlite3` handles best.
- Keep the **hand-rolled migration runner**. It works, and it is idempotent (safe to re-run without side effects). `src/storage/db.ts` resolves `migrations/` relative to `__dirname` — preserve that layout in any future build or monorepo change, since the SQL files must land next to the compiled `db.js`.
...
The backend's TypeScript version is one major behind the frontend, and TypeScript 6's new defaults need to be set explicitly to keep the backend's current behavior unchanged.

**Today (backend):** `typescript ^5.7.3`. `tsconfig.json` sets `module: NodeNext` and `moduleResolution: NodeNext` (TypeScript's mode for resolving imports the way Node itself does, rather than the way a bundler like Vite would), `target: ES2022`, and `strict: true`, and emits compiled output to `dist/`. Imports use explicit `.js` extensions, which is correct for NodeNext-mode ESM. **Frontend** is already on `typescript ^6.0.3`.

**Findings (web-verified):**

- **TypeScript 6.0** shipped **2026-03-23**. It is a transition release toward the Go-based 7.0 ("Project Corsa"). Its new defaults are `strict: true`, `target: es2025`, `module: esnext`, and `moduleResolution: bundler` — and, importantly, the `types` option now **defaults to `[]`**, so `"types": ["node"]` must be set explicitly to keep Node's global types available. The older `moduleResolution: node` setting (also called node10) is now **deprecated**; use `nodenext`, `node20`, or `bundler` instead. Sources: TS 6.0 release notes; devblogs, "Announcing TypeScript 6.0"; the privatenumber 5.x→6.0 migration gist.
- **Native TS type-stripping in Node is now Stable** (Node **25.2.0 / 24.12.0**). Type-stripping means Node can run a `.ts` file directly by deleting its type annotations, without a separate compile step. Node **26 removed the older `--experimental-transform-types` flag** entirely. Node only strips "erasable" syntax — it doesn't support `enum`, parameter properties, or runtime `namespace` — and it ignores `tsconfig.json` entirely. TypeScript's `--erasableSyntaxOnly` flag (added in 5.8) makes `tsc` reject non-erasable syntax, so code can be kept runnable this way. Sources: nodejs.org TypeScript docs; "Running TypeScript Natively."

**Recommendation:**

- **Unify on TypeScript 6.0** across both halves: set the root `devDependency` to `^6`. The frontend is already there; the version gap is itself a hazard for any type shared between them.
- **Keep `module`/`moduleResolution` set to `NodeNext` on the backend** — it targets Node directly, not a bundler. Do **not** switch the backend to `bundler` resolution.
- Make the new TypeScript 6 defaults explicit in `tsconfig.json`, so the upgrade changes nothing in practice: add `"types": ["node"]`, keep `strict`, set `target` to `es2023`/`es2024` (matching Node 24), and set `verbatimModuleSyntax: true` to enforce disciplined `import type` usage.
- **Keep the `.js` import extensions.** They are required by NodeNext ESM and by native type-stripping. Removing them would only be possible under `bundler` resolution, which this plan explicitly does not adopt for the backend. _This is a "do not change."_
- Optionally add **`erasableSyntaxOnly: true`**, keeping open the option of running `.ts` files directly with `node`, and of eventually dropping `tsx` from the runtime path. First check the codebase for `enum`/`namespace`/parameter-property usage; adopt this flag only if the codebase is already erasable.

**Native execution / `tsx`:** `tsx` (`^4.19`) remains the practical choice for local development, since Node's native type-stripping ignores `tsconfig` path aliases and won't run `.tsx` files. Keep `tsx` for the `dev:*` scripts; it can be dropped from production, since `start:*` already runs the compiled output via `node dist/...`.

**Risk:** Low–Medium — mostly making already-implicit configuration explicit. **Effort:** Medium. **Priority:** Medium.
...
- Run the codemod, then hand-fix: the backend's schemas are small (`ScoringResultSchema`, `BatchSchema`, the monitor/settings/user route inputs). Audit each `z.enum`, `.default()`, and any custom error messages.
- Standardize imports on the bare `zod` v4 entrypoint across both packages, avoiding a mix of `zod` and `zod/v4` import paths once everything is on 4.

**Risk:** Medium (semantic `.default()`/error changes can pass type-check but change runtime validation — cover with tests). **Effort:** Medium (small surface, codemod-assisted). **Priority:** **High** (unblocks shared code; a performance and consistency win on its own).

---

## 7. Frontend stack — already current, mostly "do not change"

The frontend needs no version upgrades. Its one real issue is a duplicated type definition that has already drifted from the backend.
...
**Findings (web-verified):** These are the mid-2026-current majors. shadcn/ui's (a UI component library) current model is the CLI registry approach (`npx shadcn@latest add ...`), `new-york` style as default, `data-slot` attributes, `sonner` replacing the old `toast`, and `forwardRef` removed since React 19 no longer needs it — consistent with what's already in `web/src/components/ui`. Tailwind 4 uses the CSS-first `@theme` model via the Vite plugin (no `tailwind.config.js` PostCSS chain) — also already in place. An emerging "latest of the latest" option is **`rolldown-vite`** (Vite running on the Rolldown bundler instead of esbuild) and Oxlint/Oxfmt, seen in fresh 2026 starters.
...
The scraper's stealth-browser layer needs both a routine version bump and a deeper rework, because the technique it currently relies on to avoid bot detection is becoming outdated.

**Today:** `playwright ^1.49.1`, `playwright-extra ^4.3.6` + `puppeteer-extra-plugin-stealth ^2.11`, `impit ^0.5` (a lightweight HTTP client library), `scrapfly-sdk ^0.10.6`, `cheerio ^1.0` (an HTML-parsing library), a ScrapFly fallback (a managed anti-bot-bypass service), per-platform scrapers (ebay/grailed/vestiaire/depop/poshmark) plus a Playwright stealth-browser layer.
...
- **The `puppeteer-extra-plugin-stealth` approach is now considered last-resort and brittle.** Modern anti-bot systems (Cloudflare, DataDome) detect the **`Runtime.enable` CDP leak** that plain Playwright/Puppeteer emit. CDP is the Chrome DevTools Protocol — the interface these tools use to control a browser — and `Runtime.enable` is a call within it that leaves a detectable fingerprint. Current mitigations: **Patchright** (patches the CDP leak at the browser launch/binary level; `rebrowser-patches` was also considered but dropped after a 2026-07-18 benchmark showed it now ties unpatched vanilla Playwright), Camoufox (an anti-detect build of Firefox), or driverless approaches. Sources: rebrowser/rebrowser-patches; rebrowser.net Runtime.Enable writeup; decodo/browserless/bug0 scraping guides.
...
- **Re-evaluate the stealth layer.** Treat `playwright-extra` + `puppeteer-extra-plugin-stealth` as on the way out; pilot **Patchright** for the Playwright browser path to address the `Runtime.enable` leak (`rebrowser-patches` dropped from consideration — see `docs/playwright-stealth-pilot.md` for the full pilot writeup). Keep coherent fingerprints (UA/client-hints/viewport/timezone aligned) and human-like pacing — much of `src/lib/user-agent.ts` / `src/platforms/playwright` already does this.
...
- **pnpm 10** is the 2026 default for teams/monorepos: strict dependency isolation (a package can't accidentally use another package's dependency it never declared — this surfaces so-called phantom dependencies), a content-addressable store (~30% disk vs npm), the `workspace:*` protocol and version catalogs (both explained in the companion monorepo plan), graph-aware `--filter`, and first-class Turborepo/Nx support. **Bun 1.2** is fastest to install but trades isolation and has monorepo edge cases; **npm 11** workspaces are "fine for small repos" but lack topological/affected commands. Sources: PkgPulse pnpm-vs-bun-2026, pnpm-vs-npm-vs-yarn-vs-bun-2026; Steve Kinney workspace course.
- **Vitest 4** is current. **ESLint 10** (flat config) is current; **Biome 2** is the leading all-in-one (lint+format) for new apps; **Oxlint** is the fastest lint-only pre-pass. Consensus: **keep ESLint for mature, type-aware, plugin-heavy codebases; optionally add Oxlint as a fast CI pre-pass**; Biome is most attractive on greenfield (brand-new) projects. Sources: PkgPulse/BuildPilot 2026 linter comparisons; youngju.dev 2026 formatters/linters.
...
**Risk:** Low–Medium (pnpm's stricter install layout can surface phantom-dependency errors — that's the intended effect, but it can break a sloppy import that happened to work under npm's looser hoisting). **Effort:** Medium. **Priority:** Medium.

---

## 11. Recommended sequenced upgrade order

Do these steps in order. Each one should land with a green typecheck, test run, and lint pass before starting the next. Steps 1–3 are pure foundation; step 4 is the keystone; steps 5–7 are independent and can run in parallel; step 8 sets up the monorepo.
