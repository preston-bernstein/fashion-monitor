...
- The desktop has no bundled TLS (HTTPS encryption) proxy of its own for this app. The old NAS
  setup ran its own `proxy`/Caddyfile service (Caddy is a reverse-proxy server that terminates
  HTTPS), but that would conflict with the desktop's existing shared Caddy instance, which
...
- **Scrape/score split (2026-07-19):** the pipeline is now two separate CLI entry points
  ...
  `scraper-egress` VPN tunnel (`network_mode: container:gluetun-scraper` — Gluetun is the VPN
  container the traffic routes through) and never touch the LLM. The new `score` service stays
  off that VPN tunnel so it can reach the LAN's Ollama broker (the shared local LLM server), and
